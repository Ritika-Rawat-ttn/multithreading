...
public class deadlock {
    public static void main(String[] args) throws InterruptedException {
        deadlock reentrantLockExample = new deadlock();
        Account account1 = new Account("4875984", 40000);
        Account account2 = new Account("8743587", 20000);
    ...
    private void lock(Account fromAccount, Account toAccount) {
        while (true) {
            ...
            if(fromAccountLocked) {
                ((Lock) fromAccount.getLock()).unlock();
            }
            if(toAccountLocked) {
                ((Lock) toAccount.getLock()).unlock();
            }
            try {
                Thread.sleep(1000);
